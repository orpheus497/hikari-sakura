...
| **FB-3** | **Hybrid Intel + NVIDIA with `hw.nvidiadrm.modeset=1`** — two KMS devices enumerated; wlroots picks one unaided | **OPEN — new in Phase 70, prime suspect for FB-4** | Unhandled. `card0` = Intel Iris Xe (eDP-1), `card1` = NVIDIA GTX 1650 Ti | W0-1 (`WLR_DRM_DEVICES=/dev/dri/card0`). Then: honour an explicit device in `start-hikari.sh`, or select the device that actually has connected outputs. Document the `nvidia-drm` interaction. See §5 H0 |
| **FB-4** | eDP-1 scanout swapchain test failure | **OPEN since Phase 19** | Logged loudly since Phase 25; output left disabled, session survives on the noop output | Likely subsumed by FB-3. Otherwise H1/H2/H3 via W0 runs 3–5. See §5 |
...
| **FB-6** | `explicit_bzero` / `setgroups` / `usleep` need `__BSD_VISIBLE`, which FreeBSD's `<sys/cdefs.h>` clears whenever `_POSIX_C_SOURCE` is defined | **OPEN — awaiting a user decision (Phase 72)** | `WITH_POSIX_C_SOURCE=YES` yields exactly 3 implicit-declaration warnings, reproduced Phase 72 — two security-relevant (`explicit_bzero`, `lock_mode.c:71`, wipes the password buffer; `setgroups`, `server.c:1209`, privilege drop) — and fails `-Werror` under `DEBUG=YES`. `lock_mode.c`'s existing shim is guarded `!defined(__FreeBSD__)` and so **never fires for this case** | **Opt 1 (recommended): retire the flag** — 4 lines, removes a permanently-broken config that `WITH_ALL` never sets; strict-POSIX namespace enforcement has no consumer in a FreeBSD-only compositor. **Opt 2: keep and fix** with three `__BSD_VISIBLE`-guarded declarations across three files. Held rather than guessed: Opt 2 is a workaround spreading over three files (against the standing anti-debt directive), Opt 1 removes a feature (against AGENTS.md §3) |
| **FB-7** | NVIDIA proprietary driver present on the host | **Note only, no action** | Not a project dependency | AGENTS.md §2 governs *project* dependencies; a user's system driver is out of scope. But hikari must not *require* it — FB-3's fix should prefer the Intel device |
...
**Assessment (Phase 70, updated Phase 72).** Of the nine rows, **FB-2 and FB-8 are now resolved**, FB-6 awaits a one-line user decision, and only **FB-3 / FB-4** remain genuine open platform defects — with FB-3 most likely a *configuration* problem rather than a code one. FB-1 and FB-2 had drifted into being cited as hikari-side FreeBSD hacks and were neither; that record is now corrected both here and in the code comment that originally asserted it. The overall FreeBSD-native debt is therefore **much smaller than the accumulated prose suggested**, and the remaining risk concentrates in one hardware-configuration question that a single environment variable can answer — which `hikari_platform_log()` now names in the log, beside the symptom, whenever more than one GPU is present.
