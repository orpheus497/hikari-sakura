...
| Phase 3 | FreeBSD Native Header Adaptations & Toolchain Flags | In Progress | 50% |
| Phase 4 | Data-Oriented Design (DOD) Struct-of-Arrays (SoA) Refactoring | Specified | 30% |
...
- [ ] Replace `<linux/input-event-codes.h>` with FreeBSD native `<dev/evdev/input-event-codes.h>` header in `src/binding_config.c`, `src/configuration.c`, and `src/pointer_config.c`
